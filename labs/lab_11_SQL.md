...
In the repository created by GitHub Classroom, modify the `queries.sql` file. It will be auto-graded on each commit.
...
If you are curious, this database was initialized from the script in [t177.sql](../examples/sql/t177.sql).
...
To run a query on sqliteonline.com, press `Shift+Enter` on the keyboard or click the green **Run** (▶) button at the top of the page.
...
2. **Earliest Assignment Due Date**  
   Write a query to find the earliest assignment due date in the `assignments` table. Hint: use the `MIN()` aggregate function on `due_date`.
...
6. **Find the Most Recent Completed Assignment**  
   Write a query to find the most recent `due_date` among assignments with status `'Completed'`.

7. **Count of "Not Started" Assignments**  
   Write a query to return the total number of assignments with status `'Not Started'`. Remember to use single quotes for string literals in SQL.
